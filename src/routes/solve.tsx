...
export const Route = createFileRoute("/solve")({
  head: () => ({
    meta: [
      { title: "Photo Solver — Student Helper" },
      { name: "description", content: "Prends en photo un exercice et reçois la solution détaillée étape par étape." },
    ],
  }),
  component: SolvePage,
});
...
function SolvePage() {
  ...
  const solve = async () => {
    ...
    try {
      ...
      // Save to history (image to storage + row in solved_exercises)
      if (acc.trim() && user) {
        try {
      ...
      <header className="border-b bg-card/70 backdrop-blur sticky top-0 z-10">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" size="sm" onClick={async () => { await signOut(); navigate({ to: "/" }); }}>Déconnexion</Button>
          </div>
        </div>
      </header>
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <Button
                onClick={solve}
                disabled={streaming}
                className="flex-1 bg-gradient-hero shadow-soft h-11"
              >
                {streaming ? "Analyse en cours..." : "✨ Résoudre cet exercice"}
              </Button>
              <Button variant="outline" onClick={reset} disabled={streaming} className="h-11">
                Changer
              </Button>
            </div>
          </div>
